...
export default function App() {
  ...
  const [state, setState] = useState<GameState>(() => newGame(makeConfig(playerCount, seed)));
  const [selectedId, setSelectedId] = useState<string | null>(null);

  const [overlayVisible, setOverlayVisible] = useState<boolean>(false);
  const [overlayText, setOverlayText] = useState<{ title: string; subtitle: string; btn: string }>(() => ({
    title: "Pass Device",
    subtitle: "Hand the device to the next player.",
    btn: "Continue",
  }));

  // When selection changes, keep sidebar useful (no additional logic required)
  ...
  function onAddOrder(order: Order) {
    setState((s) => {
      const next = {
        ...
        ordersByPlayer: {
          ...s.ordersByPlayer,
          [order.playerId]: [...(s.ordersByPlayer[order.playerId] ?? []), order],
        },
      };
      // Keep supply fresh for validation/UI (ownership hasn't changed in planning)
      next.suppliedByTerritory = computeSuppliedByTerritory(next.ownerByTerritory, next.capitalByPlayer);
      return next;
    });
  }
  ...
  function onRemoveOrder(playerId: PlayerId, index: number) {
    setState((s) => {
      ...
      list.splice(index, 1);
      return { ...s, ordersByPlayer: { ...s.ordersByPlayer, [playerId]: list } };
    });
  }
  ...
  function onConfirmPlayer(playerId: PlayerId) {
    setState((s) => {
      ...
      const nextState = { ...s, planningConfirmed: confirmed, currentPlanner: nextPlanner };
      return nextState;
    });

    // show pass overlay if there's another player to plan
    setTimeout(() => {
      setState((s) => {
        ...
        <div className="card vstack">
          <div className="hstack" style={{ justifyContent: "space-between" }}>
            ...
            <div className="hstack">
              ...
              <button className="btn" onClick={onNewGame}>Start / Reset</button>
            </div>
          </div>
